...
public class CarreraAssembler implements RepresentationModelAssembler<Carrera, EntityModel<Carrera>> {
    ...
    public EntityModel<Carrera> toModel(Carrera carrera) {
        ...
        carreraModel.add(linkTo(methodOn(CarreraController.class).getCarreraByCodigo(carrera.getCodigo())).withSelfRel());
        carreraModel.add(linkTo(methodOn(CarreraController.class).getAllCarreras()).withRel("carreras"));
        carreraModel.add(linkTo(methodOn(SalaController.class).getAllSalas()).withRel("salas"));
